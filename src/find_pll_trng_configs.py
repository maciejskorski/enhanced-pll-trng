...
def add_variables(problem, family, nb_PLLS, fixed_m0=0, fixed_n0=0, fixed_c0=0, fixed_pvco0=0):
    ...
    (
        ...
        pvcomin,
        pvcomax,
    ) = get_physical_contraints(family)
...
def add_physical_constraints(problem, family, nb_PLLs):
    ...
    constraints = [
        ...
        (
            lambda m1, n1, c1, pvco1: fpll1_min <= fref * m1 * pvco1 / (n1 * c1) <= fpll1_max,
            ("m1", "n1", "c1", "pvco1"),
        ),
    ]
    if nb_PLLs == 2:
        ...
        constraints += [
            ...
            (
                lambda m0, n0, c0, pvco0: fpll0_min <= fref * m0 * pvco0 / (n0 * c0) <= fpll0_max,
                ("m0", "n0", "c0", "pvco0"),
            ),
        ]
...
def add_application_requirements(
    ...
    if d_min != 1 or d_max != float("inf"):
        constraints += [
            (
                lambda m0, n0, c0, m1, n1, c1: d_min <= distances(jit, m1 * n0 * c0, m0 * n1 * c1) <= d_max,
                ("m0", "n0", "c0", "m1", "n1", "c1"),
            )
        ]
    if rs_min != 0 or rs_max != float("inf"):
        constraints += [
            (
                lambda m0, n0, c0, m1, n1, c1: rs_min <= rs_product(m0 * m1, n0 * n1 * c1 * c0) <= rs_max,
                ("m0", "n0", "c0", "m1", "n1", "c1"),
            )
        ]
...
def extend_solutions(solutions, fref):
    ...
    for solution in solutions:
        ...
        d = distances(
            jit, solution["m1"] * solution["n0"] * solution["c0"], solution["m0"] * solution["n1"] * solution["c1"]
        )
        solution["#pts"] = len(d)
...
def save_solutions_to_csv(solutions, csv_filename, fref, nb_PLLs):
    ...
    if nb_PLLs == 2:
        with open(csv_filename, "w") as csv_file:
            csv_file.write(
                "fref,m0,n0,c0,pvco0,m1,n1,c1,pvco1,Km,Kd,f0,f1,fpfd0,fpfd1,fvco0,fvco1,R,S,#pts,#>20,#>30,#>40,#>50,dmean,d\n"
            )
            for solution in solutions:
                csv_file.write(
                    "{},{m0},{n0},{c0},{pvco0},{m1},{n1},{c1},{pvco1},{Km},{Kd},{f0},{f1},{fpfd0},"
        ...
        with open(csv_filename, "w") as csv_file:
            ...
            for solution in solutions:
                csv_file.write(
                    "{},{m1},{n1},{c1},{pvco1},{Km},{Kd},{f0},{f1},{fpfd0},"
                    "{fpfd1},{fvco0},{fvco1},{R},{S},{#pts},{#sup20},{#sup30},{#sup40},{#sup50},{dmean},{d}\n".format(
                        fref, **solution
                    )
                )
...
if __name__ == "__main__":
    ...
    problem, constraints = add_physical_constraints(problem, family, nb_PLLs)
    problem, constraints = add_TRNG_setting_constraints(problem, constraints, family, nb_PLLs, s_m, s_d)
    problem = add_application_requirements(
        problem,
        constraints,
        family,
        nb_PLLs,
